  ...
  export default {
        data () {
            return {
                ...
                ruleInline: {
                    user: [
                        { required: true, message: '快快输入用户名,时间不等人呐!', trigger: 'blur' },
                         { type: 'string', max: 11, min:5, message: '用户长度最少为5位,最多11位哦,（＞人＜；）', trigger: 'blur' }
                    ],
                    password: [
                        { required: true, message: '还有密码密码密码!', trigger: 'blur' },
                        { type: 'string', min: 8, max:16, message: '密码长度最少为8位,最多16位哦,（＞人＜；）', trigger: 'blur' }
                    ],
                     email: [
                        { required: true, message: '这里填你的qq号鸭,我会给你发邮箱验证的', trigger: 'blur' }
                    ],
                    value: [
                        { required: true, message: '你还没有输入验证码,不要着急！', trigger: 'blur' }
                    ]
                }
            }
        },
        ...
        methods: {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        if(this.isshow) {
                            ...
                            PostMessage('/user/login',this.formInline)
                            .then( res => {
                            if(res.data.err == 0) {
                        ...
                        /* 默认走登陆模式 */
                            ...
                            if(res.data.err == 0) {
                                ...
                                this.$Message.error('没有查询到小主的信息哦需要进行注册, ⊙﹏⊙∥！')
                            }
                        })
                        }
                    } else {
            ...
            if(this.formInline.email != '') {
                ...
                if(reg.test(this.formInline.email)==true) {
                    /* 发起验证码请求 */
                    PostMessage('/user/emailInfo',{email:this.formInline.email+'@qq.com'})
                    .then(res => {
                        if(res.data.err == 0) {
                            /* 发送成功 */
